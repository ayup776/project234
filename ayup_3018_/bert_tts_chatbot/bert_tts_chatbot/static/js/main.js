...
function confidenceTier(score) {
  if (score === null || score === undefined) return "open";
...
function appendBotMessage({ response, intent, confidence, audio_url }) {
  ...
  const pillLabel = tier === "open" ? "open-domain" : `${Math.round(confidence * 100)}%`;

  row.innerHTML = `
    <div class="msg-bubble"></div>
    <div class="msg-meta">
      <span class="intent-tag">${intent}</span>
      <span class="confidence-pill ${tier}">${pillLabel}</span>
    </div>
