# BERT + Generative Voice Chatbot (Flask)

A text-in, voice-out chatbot: known topics get instant, consistent
replies via BERT-based intent matching; anything outside that gets a
real generated answer from a small language model instead of a
"sorry, I didn't understand." Every reply is spoken back using a
neural text-to-speech model (SpeechT5), all served through Flask.

## How it works

1. **Intent matching (fast path):** `chatbot/bert_model.py` embeds
   every example phrase in `chatbot/intents.json` using
   `sentence-transformers` and compares it to the user's message via
   cosine similarity. If there's a confident match (score >= 0.6), a
   response from that intent is used -- instant and predictable for
   common things like greetings, "how are you," "tell me a joke," etc.
2. **Generative fallback (open-ended):** `chatbot/generative_model.py`
   loads `Qwen/Qwen2.5-0.5B-Instruct`, a small instruction-tuned
   language model. When intent matching isn't confident about
   anything, the user's message goes to this model instead, which
   generates a genuinely new response rather than picking from a
   fixed list. This is what lets the assistant answer things it was
   never explicitly programmed to handle.
...
The first run downloads `all-MiniLM-L6-v2` (~90MB), `speecht5_tts` and
`speecht5_hifigan` (~600MB combined), `Qwen2.5-0.5B-Instruct` (~1GB),
plus one speaker embedding. Needs internet access once; everything is
cached locally afterward. Budget a few GB of disk space.
...
loud. On CPU, intent-matched replies are near-instant, but generative
fallback replies (anything outside the known intents) take longer --
the language model has to generate each word one at a time. Repeated
identical responses reuse the cached audio file instantly either way.
