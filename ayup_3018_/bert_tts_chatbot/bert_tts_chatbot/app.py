...
from chatbot.generative_model import GenerativeChatbot
...
# Loaded once at startup -- kept in memory for the lifetime of the server
# so every request is fast (well, as fast as CPU inference allows).
chatbot = BertChatbot(os.path.join(BASE_DIR, "chatbot", "intents.json"))
generative_chatbot = GenerativeChatbot()
...
def chat():
    ...
    # BERT's intent matching only covers what's defined in intents.json.
    # Anything it isn't confident about gets routed to the generative
    # model instead, so the assistant can respond to open-ended
    # questions rather than only ever falling back to a static message.
    if intent == "unknown":
        bot_response = generative_chatbot.generate_reply(user_text)
        intent = "generative"
        confidence = None
